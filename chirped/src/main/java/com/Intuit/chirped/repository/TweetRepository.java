...
//    Page<Tweet> findLatestTweetsByUserAndFollowers(@Param("userId") Integer userId, Pageable pageable);
    ...
    Page<Tweet> findLatestTweetsByUserAndFollowers(@Param("userId") Integer userId, @Param("lastSeenTimestamp") Date lastSeenTimestamp, Pageable pageable);


    @Query("SELECT t FROM Tweet t WHERE t.user.userId = :userId AND (t.createdAt <= :lastSeenTimestamp OR t.updatedAt <= :lastSeenTimestamp) ORDER BY COALESCE(t.updatedAt, t.createdAt) DESC")
    Page<Tweet> findLatestTweetsByUser(@Param("userId") Integer userId, @Param("lastSeenTimestamp") Date lastSeenTimestamp, Pageable pageable);

    @Query("SELECT t FROM Tweet t WHERE t.user.username = :username AND (t.createdAt <= :lastSeenTimestamp OR t.updatedAt <= :lastSeenTimestamp) ORDER BY COALESCE(t.updatedAt, t.createdAt) DESC")
    Page<Tweet> findLatestTweetsByUsername(@Param("username") String username, @Param("lastSeenTimestamp") Date lastSeenTimestamp, Pageable pageable);
